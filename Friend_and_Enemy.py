...
class Friend(arcade.Sprite):
    ...
    def __init__(self, main_path, scale, wall_list, player_sprite):
        ...
        for i in range(1, 3):
            self.front_texture_list.append(arcade.load_texture(f"{main_path}_front_{i}.png",
        ...
        """Other things"""
        self.barrier_list = arcade.AStarBarrierList(self, wall_list, TILE_SIZE,
    ...
    def update_path(self, player_sprite, delta_time):
        self.path = arcade.astar_calculate_path(self.position,
                                                (player_sprite.position[0]+TILE_SIZE,
                                                 player_sprite.position[1]+TILE_SIZE),
                                                self.barrier_list,
...
class Enemy(arcade.Sprite):
    ...
    def on_update(self, physic_engine: Optional[arcade.PymunkPhysicsEngine], delta_time, bullet_list):
        ...
        if self.time_in_sight > TIME_TO_SEE:
            ...
            if self.attack_cooldown <= 0:
                ...
                bullet = BulletSprite(str(os.path.dirname(
                                      os.path.abspath(__file__)))+r"\new_assets\user_int\fireball.png",
                                      self,
    ...
    def return_to_path(self):
        if self.return_path is None:
            ...
            tmp_path = arcade.astar_calculate_path(self.position,
                                                   self.static_path[self.cur_dir],
                                                   self.barrier_list,
                                                   diagonal_movement=True)
        ...
        if round(self.center_x) == round(self.return_path[self.return_path_position][0]) and\
                round(self.center_y) == round(self.return_path[self.return_path_position][1]):
    ...
    def update_path(self, delta_time):
        ...
        if int(self.center_x) == int(self.static_path[self.cur_dir][0]) and\
                int(self.center_y) == int(self.static_path[self.cur_dir][1]):
            self.cur_dir += 1
